...
const Datatable = (props) => {
    ...
    const selectActions = [
        {
            ...
            button: () => (
                ...
                >
                    Add Job <PlusIcon className="ml-2 h-5 w-5" aria-hidden="true" />
                </button>
            ),
        },
        {
    ...
    const updateRecord = async (item) => {
        const dta = await fetcher.update(item, false, setLoadingUpdate);
        if (dta) {
    ...
    const deleteRecord = async () => {
        const dta = await fetcher.removeBulk(selectedData, false, setLoadingDelete);
        if (dta) {
    ...
    return (
        <>
            ...
            <SidePanel title={`Edit Job '${editingData?._id}'`} setOpen={setEditingOpen} isOpen={editingOpen}>
                {editingData && (
                    <Add
                        data={editingData}
                        edit={true}
