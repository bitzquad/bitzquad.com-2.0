            ...
            <div data-scroll-section className="bz-container relative mx-auto mt-0 h-full w-full bg-gray-50 md:bg-transparent lg:mt-44">
                <div className=" py-10 lg:px-5 lg:py-0">
                    ...
                    <p className="mt-4 text-sm  tracking-widest text-gray-700 lg:mt-5 lg:text-xl xl:w-7/12">
                        ...
                        <br></br>
                        We admire and respect professionalism, integrity, sportsmanship, transparency and modesty. And we aspire to cultivate these qualities within the work environment as we grow.
                    </p>
                    <div className="mt-16 flex flex-row flex-wrap gap-x-6 text-2xl">
                        {types.map((v) => (
                            <motion.span key={v.value} className={v.value == selCat ? "underline" : "underline-link"} data-cursor="-opaque" whileHover={{ scale: 1.05 }} transition={{ duration: 0.4, ease: [0.65, 0.05, 0.36, 1] }} onClick={() => setSelCat(v.value)}>
                                {v.name}
                            </motion.span>
                        ))}
                    </div>
                </div>
                <motion.div className="mb-28 grid grid-cols-2 gap-x-4 gap-y-3 md:grid-cols-3  lg:mt-20 lg:grid-cols-4 lg:gap-x-6  lg:gap-y-10 lg:px-10">
                    {members
    ...
    return (
        <Link href={`/squad/${member.id}`}>
            <motion.div className="group cursor-pointer">
                ...
                >
                    <motion.img src={member.img} className=" w-full object-cover" whileHover={{ scale: 1.05 }} transition={{ duration: 0.3, ease: "easeInOut" }} />
                </motion.div>
                <div className="">
                    <h3 className="mt-1 text-base font-bold uppercase lg:mt-4 lg:text-3xl ">{member.firstName}</h3>
                    <h3 className="mt-0 text-base font-bold uppercase lg:text-xl">{member.lastName}</h3>

                    <h5 className=" text-sm text-gray-700 lg:text-lg">{member.role.text}</h5>
                </div>
            </motion.div>
        </Link>
    );
