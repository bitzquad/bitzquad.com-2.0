...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const authresp = apiauthresolver.resolveToken(req, res); // Resolve the job type & id
    if (authresp.success) {
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "jobpublic"); // Get the permissions for the current job
        if (!p.read.allowed) return res.status(403).json({ error: "Forbidden" }); // Check if the job has the permission to read the collection
