...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const authresp = apiauthresolver.resolveToken(req, res); // Resolve the obj type & id4
    if (authresp.success) {
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "users"); // Get the permissions for the current obj
