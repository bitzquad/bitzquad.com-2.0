...
async function handleSignin(req: NextApiRequest, res: NextApiResponse) {
    await mongodb.Connect();
    try {
        let resp = await User.findOne({ email: req.body.email, password: req.body.password, draft: false, deleted: false }, { name: 1, status: 1, "thumbnail.src": 1, email: 1 });
        if (resp) {
...
async function handleSignup(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        let eu = await User.findOne({ password }, { name: 1 });
        if (eu) {
